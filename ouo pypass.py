...
url = input("Nhập link ouo.io của bạn:")
# -------------------------------------------
...
def RecaptchaV3():
    ...
    matches = re.findall(r'([api2|enterprise]+)\/anchor\?(.*)', ANCHOR_URL)[0]  # Fixed the regex by adding `r` for raw string
    url_base += matches[0]+'/'
    params = matches[1]
...
def ouo_bypass(url):
    ...
    for _ in range(2):
        ...
        bs4 = BeautifulSoup(res.content, 'html.parser')  # Use the 'html.parser' if 'lxml' is not available
        inputs = bs4.find_all("input", {"name": re.compile(r"token$")})  # Changed to raw string
